...
public enum MongoDBDriverConnectionStringPattern {
	...
	PATTERN_THREE(
			"^(mongodb://)([a-zA-Z]+[a-zA-Z0-9.]+@[a-zA-Z0-9.]+)/([a-zA-Z0-9_]+)/([a-zA-Z]+)");
	...
	public void checkConnectionString(String connectionString)
		...
		if (connectionString == null)
			throw new InvalidParameterException(
					"connection string can not be null");
		if (connectionString.trim().length() == 0)
			throw new InvalidParameterException(
					"connection string can not be zero length");
	}
	...
	public MongoDBDriverParams parseForPatternOne(String connectionString)
		...
		throw new InvalidParameterException(
				"Connection string does not match with pattern");
	}
